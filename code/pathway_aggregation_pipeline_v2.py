"""
BIOADAPT-ARCHETYPE v6 — Pathway Aggregation Pipeline (v2)
=========================================================
Mixed-cohort nested CV with full v4 feature parity.
...
FIX-ARCH-TUNE: Membership-weighted archetype tuning per model per objective.

TRAIN EVAL (unbiased, consensus pipeline pattern):
  Within each outer fold, 20% of the outer-fold training set is withheld
  BEFORE any training begins. All tuning and training runs on the remaining
  80% (X_dev). The withheld 20% (X_val) is evaluated after. The full outer
  training set is then used separately for the final test evaluation.
  CSV type labels:
    Test_Routed / Test_Baseline           — outer-fold test split
    Train_Routed_HoldOut / Train_Baseline_HoldOut — within-train unbiased eval
"""
...
F2F3_MIN_SPEC      = 0.50
TRAIN_HOLDOUT_FRAC = 0.20


# ─── LOGGING ─────────────────────────────────────────────────────────────────
def setup_logging(label='pathway_agg_v2'):
    cfg.OUTPUT_DIR.mkdir(exist_ok=True, parents=True)
    log = logging.getLogger('bioadapt_pathway_v2')
...
def get_logger(): return logging.getLogger('bioadapt_pathway_v2')
...
def _pipeline_eval(
    ...
    """
    Full pipeline for one train→test split. Called twice per outer fold:
      (1) X_dev → X_val  (unbiased within-training eval, train holdout)
      (2) X_pw_tr → X_pw_te  (outer-fold test eval)
    ...
    # Router — fit on training portion only
...
# ─── OUTER FOLD ───────────────────────────────────────────────────────────────
def run_outer_fold(fold_id, tr_idx, te_idx, X_raw, y, cohort, genes):
    ...
    # TRAIN EVAL: stratified holdout reserved before any training
    # fold_id in seed ensures different splits across outer folds
    ...
    log.info(f"Train holdout split: dev n={len(dev_idx)} "
    ...
    # Pipeline call 2: full outer-fold train → outer-fold test
    log.info("\n[Running pipeline: full_train → test  (test eval)]")
    baseline_te, routed_te = _pipeline_eval(
        X_pathway_tr, ytr, atr, anames, pathway_names,
    ...
    # Save checkpoint so this fold can be loaded on resume
    cfg.OUTPUT_DIR.mkdir(exist_ok=True, parents=True)
    ckpt_path = cfg.OUTPUT_DIR / f'pathway_agg_v2_fold{fold_id}.pkl'
    with open(ckpt_path, 'wb') as f:
        pickle.dump(result, f)
    log.info(f"Fold {fold_id+1} checkpoint saved → {ckpt_path}")
    return result
...
def run_full_pipeline(folds_to_run=None):
    ...
    folds_to_run : list of 0-based fold indices to run, e.g. [2, 3].
                   ...
                   Folds not in the list are loaded from per-fold checkpoints
                   (pathway_agg_v2_fold{N}.pkl) if available, otherwise skipped
                   with a warning and excluded from the summary.

    CLI usage:
        python pathway_aggregation_pipeline_v2.py          # run all folds
        python pathway_aggregation_pipeline_v2.py 2 3      # run folds 2 and 3 only
    """
    log = setup_logging(); t0 = time.time()
    log.info("="*62)
    log.info(" BIOADAPT-ARCHETYPE v6 — PATHWAY AGGREGATION (v2)")
    log.info(f" Models        : {PIPELINE_MODELS}")
    log.info(f" Calibration   : {CALIBRATION_MODES}")
    log.info(f" FIX-1         : F2/F3 min Spec >= {F2F3_MIN_SPEC}")
    log.info(f" Train holdout : {int(TRAIN_HOLDOUT_FRAC*100)}% of each outer fold")
    if folds_to_run is not None:
        log.info(f" Folds to run  : {folds_to_run}  (others loaded from checkpoint)")
    log.info("="*62)

    X, y, cohort, genes, norm = load_pooled()
    cohort_idx   = {c: i for i, c in enumerate(np.unique(cohort))}
    strat_labels = y*100 + np.array([cohort_idx[c] for c in cohort])
    outer_cv     = StratifiedKFold(n_splits=cfg.N_OUTER_FOLDS, shuffle=True,
                                   random_state=cfg.RANDOM_SEED)

    # Build the full split list first so checkpoint-loaded folds stay aligned
    all_splits = list(enumerate(outer_cv.split(X, strat_labels)))

    fold_results = {}
    for fid, (tr, te) in all_splits:
        ckpt_path = cfg.OUTPUT_DIR / f'pathway_agg_v2_fold{fid}.pkl'
        if folds_to_run is not None and fid not in folds_to_run:
            if ckpt_path.exists():
                with open(ckpt_path, 'rb') as f:
                    fold_results[fid] = pickle.load(f)
                log.info(f"Fold {fid+1}: loaded from checkpoint {ckpt_path}")
            else:
                log.warning(f"Fold {fid+1}: not in folds_to_run and no checkpoint — "
                            f"excluded from summary")
        else:
            fold_results[fid] = run_outer_fold(fid, tr, te, X, y, cohort, genes)

    # Sort by fold id for consistent output
    ...
    cfg.OUTPUT_DIR.mkdir(exist_ok=True, parents=True)
    df.to_csv(cfg.OUTPUT_DIR / 'pathway_agg_v2_metrics.csv', index=False)
    with open(cfg.OUTPUT_DIR / 'pathway_agg_v2_results.pkl', 'wb') as f:
        pickle.dump(fold_results_list, f)
    log.info(f"\nSaved to output/pathway_agg_v2_metrics.csv")
    log.info(f"Total: {(time.time()-t0)/60:.1f} min")


if __name__ == '__main__':
    import sys
    # Optionally pass fold indices as CLI args (0-based), e.g.:
    #   python pathway_aggregation_pipeline_v2.py        # run all folds
    #   python pathway_aggregation_pipeline_v2.py 2      # run fold 2 only
    #   python pathway_aggregation_pipeline_v2.py 2 3 4  # run folds 2, 3, 4
    folds = [int(x) for x in sys.argv[1:]] if len(sys.argv) > 1 else None
